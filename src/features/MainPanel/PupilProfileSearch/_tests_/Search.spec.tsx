...
import searchInputValidation, {
  SearchInputProps
} from "../utils/SearchInputValidation";
...
describe("Search Input field tests", () => {
  describe("Search input component rendering tests", () => {
    ...
    test("should render the Search Component", () => {
      ...
      const searchInputElement: HTMLElement = getByTestId(
  ...
  it("should return false if key is Enter or click but search is not valid for path other than search", async () => {
    ...
    const mockKeyPress1: jest.SpyInstance<
      Promise<ISearchSuggestionsResultsApiResponse[]>,
      [query: string, n: number]
    > = jest.spyOn(fetchSearchSuggestions, "default");

    const { container, getByTestId }: any = render(<Search isOpen />);
  ...
  it.skip("should return false if key is Enter or click but search api gives error", async () => {
    ...
    const mockKeyPress1: any = jest
      .spyOn(fetchSearchSuggestions, "default")
      .mockImplementationOnce(() => Promise.reject(new Error("error message")));
  ...
  it("should return false if key is Enter or click but search is valid for path other than search", async () => {
    ...
    const mockKeyPress1: any = jest
      .spyOn(fetchSearchSuggestions, "default")
      .mockReturnValueOnce(Promise.resolve(suggestionresponse));
    const { container, getByTestId }: any = render(<Search isOpen={false} />);
    expect(getByTestId("new-search-element")).toBeInTheDocument();
    getByTestId("new-search-element").focus();
    fireEvent.change(container.getElementsByTagName("input")[0], {
      target: { value: "ben" }

...
export const handleKeyPress: (props: IHandleKeyPressProps, inputValue: string) => boolean = (props: IHandleKeyPressProps, inputValue: string) => {
  const { e,  inputText, pagePath }: IHandleKeyPressProps = props;
  const userInput: string = inputValue.trim();
  let searchValidity: boolean;

  if (pagePath === "/search")
    searchValidity = userInput !== inputText;
  else
    searchValidity = true;

...
const fetchSearchSuggestions: (
  ...
  new Promise<ISearchSuggestionsResultsApiResponse[]>(
    async (
      resolve: (
        value:
          | ISearchSuggestionsResultsApiResponse[]
          | PromiseLike<ISearchSuggestionsResultsApiResponse[]>
      ) => void,
      reject: (reason?: any) => void
    ) => {
      try {
        const response: AxiosResponse<any, any> = await service.get(
          `/suggestions?q=${query}&n=${n}`,

...
export const setDefaultAndCurrentSlide = (
  ...
  const Index = findCurrentIndex(apiRegsiterEventData);
...
export const renderNoRegisterMessage = (t: any) => (
 ...
 */
export const generateRegisterUrl = (item: IRegistersDetails): string => {
  return item.eventTypeCode === "AttendanceSession"
 ...
 */
export const handleRegisterClick = (item: IRegistersDetails): void => {
  const url = generateRegisterUrl(item);
...
const isEventInProgress = (eventStart: string, eventEnd: string, currentUTCDateTime: string): boolean => 
  ...
  const isEventUpcoming = (eventStart: string, currentUTCDateTime: string): boolean => 
     Date.parse(eventStart) > Date.parse(currentUTCDateTime);

export const findCurrentIndex = (
    apiRegsiterEventData: IRegistersDetails[]
  ): number => {
    const currentUTCDateTime = new Date().toISOString().split(".")[0];
  ...
  export const calculateNextSlide = (
    ...
    const slidesToShow = getSlidesToShow(); // Use helper function
    return Math.min(currentSlide + slidesToShow, totalLength);
  };

  export const calculatePreviousSlide = (currentSlide: number): number => {
    const slidesToShow = getSlidesToShow(); // Use helper function
  ...
  export const getPrimaryText = (
    ...
    const text = `${item.group.shortName!} ${
      item.room ? ` | ${item.room.roomName!}` : ""
    }`;
  
    return text.length > 16 && isMediumscreen
      ? `${text.substring(0, 16)}...`
      : text;
  };

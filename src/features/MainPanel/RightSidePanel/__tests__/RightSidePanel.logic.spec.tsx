...
import { IGroupMemberDetailsResponse } from "../../../../shared/model/SchoolDomain/responsemodels";
 ...
 const mockEmptyGroupExternalId:IRightSidePanelProps = {
 ...
 const mockEventTitleBreak:IRightSidePanelProps = {
  SchoolEventexternalId: '123',
  EventTitle: 'Break',
  RoomCode: 'Room A',
  EventStart: '2023-11-10T09:00:00.000Z',
...
describe('RigthSidePanel', () => {
    ...
    test('should not set any values if groupExternalId is "00000000-0000-0000-0000-000000000000', async () => {
      ...
      .mockImplementationOnce(() => [[], setGroupMemberDetailsData])
       ...
       render(
            <RightSidePanel
              SchoolEventexternalId={mockEmptyGroupExternalId.SchoolEventexternalId}
              EventTitle={mockEmptyGroupExternalId.EventTitle}
              RoomCode={mockEmptyGroupExternalId.RoomCode}
              EventStart={mockEmptyGroupExternalId.EventStart}
              EventEnd={mockEmptyGroupExternalId.EventEnd}
              GroupExternalId={mockEmptyGroupExternalId.GroupExternalId}
              EventPeriodNo={mockEmptyGroupExternalId.EventPeriodNo}
              togglePanel={mockEmptyGroupExternalId.togglePanel}
              isOpen={mockEmptyGroupExternalId.isOpen}
              GroupDescription={mockEmptyGroupExternalId.GroupDescription}
              StaffName={mockEmptyGroupExternalId.StaffName}
            />
          );
          await waitFor(() => {
    ...
    test('should not set any values if EventTitle is Break', async () => {
      ...
      .mockImplementationOnce(() => [[], setGroupMemberDetailsData])
       ...
       render(
            <RightSidePanel
              SchoolEventexternalId={mockEventTitleBreak.SchoolEventexternalId}
              EventTitle={mockEventTitleBreak.EventTitle}
              RoomCode={mockEventTitleBreak.RoomCode}
              EventStart={mockEventTitleBreak.EventStart}
              EventEnd={mockEventTitleBreak.EventEnd}
              GroupExternalId={mockEventTitleBreak.GroupExternalId}
              EventPeriodNo={mockEventTitleBreak.EventPeriodNo}
              togglePanel={mockEventTitleBreak.togglePanel}
              isOpen={mockEventTitleBreak.isOpen}
              GroupDescription={mockEventTitleBreak.GroupDescription}
              StaffName={mockEventTitleBreak.StaffName}
            />
          );
          await waitFor(() => {
            expect(setLoader).toHaveBeenCalledWith(false);
    ...
    test('should not set any values if EventTitle is meeting', async () => {
    ...
    test('should set values if eventtitle is not break or meeting or external is not empty', async () => {
      ...
      jest.spyOn(schoolDomainservices,"FetchGroupMemberDetailsData").mockResolvedValue(mockListofGroupExternalId);
       ...
       render(
          ...
          await waitFor(() => {
            expect(setLoader).toHaveBeenCalledWith(false);
            expect(setErrCodeMessage).toHaveBeenCalledWith(false);
            expect(setGroupMemberDetailsData).toHaveBeenCalledWith(mockSortedListofGroupExternalIdBySurname);
          });
    });
  ...
  test('should handle unsuccessful data fetch', async () => {
      ...
      jest.spyOn(schoolDomainservices,"FetchGroupMemberDetailsData").mockRejectedValue(new Error());

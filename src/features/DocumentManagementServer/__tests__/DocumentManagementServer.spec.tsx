...
//      act(() => {
//   jest.advanceTimersByTime(1000); // or whatever timeout your component uses
// });
//   await waitFor(() => expect(screen.getByText(/document about to be prepared for download/i)).toBeInTheDocument());
//     console.log("Dialog appeared, clicking Cancel...");
...
//   fireEvent.click(screen.getByText(/Prepare download/i));
// screen.debug();
//   await waitFor(() => {
...
// it("shows loader in side panel when Prepare download is clicked and hides after timeout, then closes side panel", async () => {
//   (apiService.fetchDocumentDetails as jest.Mock).mockResolvedValue({
//     statusCode: 200,
//     totalRecords: 2,
//     data: [
...
describe("DocumentManagementServerView - selection and dialog logic", () => {
  ...
  // it("calls handleCloseDialogConfirmation and closes dialog", async () => {
  //   render(<DocumentManagementServerView />);
  //   act(() => { jest.advanceTimersByTime(1000); });
  ...
  //   // Click Cancel (should call handleCloseDialogConfirmation)
  //   fireEvent.click(screen.getByText(/Cancel/i));
  ...
  // Select a checkbox to enable the action
  const checkboxes = await screen.findAllByTestId(/^check-box-row-testid-/);
  fireEvent.click(checkboxes[0]);
  ...
  it("calls selectedCheckboxIds callback when checkbox is selected", async () => {
    ...
    // After selection, "selected" count or similar UI should update
    // (Adjust this assertion to match your UI)
...
//   // Advance timers to trigger setTimeout and hide loader
//   act(() => { jest.advanceTimersByTime(1000); });

//   // Loader should disappear after timeout
//   await waitFor(() => expect(screen.queryByTestId("side-panel-header")).not.toBeInTheDocument());
// });
});

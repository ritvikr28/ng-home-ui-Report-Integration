...
export const useFetchSchoolNameData: () => Promise<ISchoolNameDataResponse | null> =
  async () => {
    try {
      const response: AxiosResponse<ISchoolNameDataResponse> =
        await service.get(`School/SchoolName`);
...
export const FetchStaffTimeTableEventsData: () => Promise<IStaffTimeTableEventsDataResponse | null> =
  async () => {
    try {
      const response: AxiosResponse<IStaffTimeTableEventsResponse[] | null> =
        await service.get(
          `StaffTimetable/StaffTimetableEvents`,
          buildApplicationUrl(apiUrls)
        );
      const { status }: { status: number } = response;
      const responseData: IStaffTimeTableEventsResponse[] | null =
        response.data;
...
export const FetchGroupMemberDetailsData: (
  ...
  try {
    const responseData: AxiosResponse<IGroupMemberDetailsDataResponse> =
      await service.get(

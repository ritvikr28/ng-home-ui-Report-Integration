...
import { getNotificationTableData, getViewData, markAsRead } from "../api";
...
jest.mock("@essnextgen/auth-ui", () => ({
    authService: {
        getUserId: jest.fn(),
        getUsername: jest.fn(),
        getAuthTokens: jest.fn(), // Add this to fix the error
    },
}));
...
describe("getNotificationTableData", () => {
    ...
    beforeEach(() => {
        ...
        (authService.getUsername as jest.Mock).mockReturnValue(mockUsername);
    });

    it("returns data on success", async () => {
        (service.get as jest.Mock).mockResolvedValue(mockResponseData);
        const result = await getNotificationTableData({ PageSize: 10, PageNumber: 1 });
...
// --- Add tests for markAsRead to cover all branches ---
describe("markAsRead", () => {
    const mockOrgId = "org-123";
    const mockBaseUrl = "http://api-url";
    const mockNotificationId = "notif-456";
